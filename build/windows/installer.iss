...
; The bundle (large-v3 + embedded LLM) compresses to ~4.6 GB, past Inno's
; ~4.2 GB single-Setup.exe ceiling (a 32-bit offset limit). Disk spanning
; splits the payload into Setup-1.bin, Setup-2.bin… slices alongside a small
; Setup.exe, so large files (model.bin is 2.88 GB) span slices freely.
; Distribute Setup.exe together with every Setup-*.bin in the same folder.
DiskSpanning=yes
