...
# CRITICAL: the line above will NOT switch backends on a venv that already
# holds llama-cpp-python — pip reports the pinned package "already satisfied"
# and keeps whatever wheel is there. A venv seeded with the plain PyPI/CPU
# wheel would then ship CPU-only (no GPU summary) even though we asked for the
# Vulkan index. Force the wheel to come from the chosen index so the compiled
# backend is deterministic. PyPI has no Windows binary wheel for this package,
# so --prefer-binary makes pip take the prebuilt wheel from $LlamaIndexUrl;
# --no-deps keeps the already-installed deps untouched.
Write-Host "==> Forcing llama-cpp-python from '$LlamaIndex' wheel index (deterministic backend)"
& $python -m pip install --force-reinstall --no-deps --no-cache-dir --prefer-binary `
    --extra-index-url $LlamaIndexUrl "llama-cpp-python>=0.3,<0.4"
if ($LASTEXITCODE -ne 0) { throw "llama-cpp-python ($LlamaIndex) reinstall failed (exit $LASTEXITCODE)" }
