        ...
        with TemporaryDirectory() as tmp:
            ...
            response = self.adapter.transcribe(
                TranscribeArguments(
                    ...
                    # ``extractive`` is the legacy synonym for ``ollama``
                    # — the adapter normalises it. Test guards both that
                    # the alias is accepted and that the response shape
                    # is what current MCP clients expect.
            ...
            self.assertIsNone(as_dict["summary_error"])
