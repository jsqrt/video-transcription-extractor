...
from app.services.writer import CleanTranscriptWriter, refined_titles_from_summary
...
class E2E1Mp4Test(unittest.TestCase):
    ...
    def test_full_text_pipeline_on_existing_transcript(self) -> None:
        ...
        chapters = build_chapters(cleaned, title_style="snippet")
        self.assertGreaterEqual(len(chapters), 1)

        summary = Summarizer(
            options=SummaryOptions(
                mode="extractive",
                per_chapter_sentences=3,
        ...
        self.assertIsInstance(summary.overview, str)
        self.assertTrue(summary.overview.strip())
        self.assertEqual(len(summary.per_chapter), len(chapters))
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(clean_path.name, "1.clean.md")
            clean_content = clean_path.read_text(encoding="utf-8")
            # Chapter headings at level 2, no speaker tags in the clean file.
            self.assertIn("## ", clean_content)
