...
class _FakeLLMClient:
    """In-memory LLM stand-in so pipeline tests don't need a real Ollama.

    The summarizer is intentionally a thin wrapper now (LLM in, markdown
    out), so the cheapest faithful test double is one that just hands back
    a canned markdown string.
    """

    def __init__(self, reply: str = "## Огляд\nКоротке резюме.\n") -> None:
        self.reply = reply
