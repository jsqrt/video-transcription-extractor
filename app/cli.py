...
def build_parser() -> argparse.ArgumentParser:
    ...
    transcribe_parser.add_argument(
        ...
        dest="initial_prompt",
        default=None,
        help="Vocabulary hint passed to Whisper. Comma-separated list of "
             "domain terms, names, and English loanwords (e.g. "
             "'пейдж, фронтенд, бекенд, деплой, мерж'). Biases the "
             "decoder toward these spellings so it stops translating "
             "loanwords into Ukrainian or fusing them with adjacent "
             "numerals. Limited to ~224 Whisper tokens (~150 words).",
    )
    transcribe_parser.add_argument(
        "--initial-prompt-file",
        dest="initial_prompt_file",
        default=None,
        help="Path to a file whose contents are used as --initial-prompt. "
             "Mutually exclusive with --initial-prompt.",
    )
    transcribe_parser.add_argument(
    ...
    transcribe_parser.add_argument(
        ...
        help="Summarization backend. 'none' disables the summary file. "
             "'ollama' (default) calls a local Ollama LLM and writes its "
             "markdown reply verbatim into <name>.summary.md. If Ollama is "
             "unreachable the summary is skipped (with a logged warning) "
             "but the transcription file is still produced.",
    )
    transcribe_parser.add_argument(
        "--no-summary-file",
        action="store_false",
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.initial_prompt and args.initial_prompt_file:
        raise CliArgumentError(
            "--initial-prompt and --initial-prompt-file are mutually exclusive"
        )


def _resolve_initial_prompt(args: argparse.Namespace) -> str | None:
    """Return the inline prompt or the contents of the prompt file.

    Returns ``None`` when neither flag is set, so the provider takes
    its no-prompt default path.
    """
    ...
    if args.initial_prompt_file:
        ...
        if not path.is_file():
            raise CliArgumentError(
                f"--initial-prompt-file not found: {path}"
            )
...
def _build_summarizer(options: SummaryOptions, logger: CliLogger) -> Summarizer:
    ...
    if options.mode == "ollama":
        ...
        if candidate.is_available():
            ...
            logger.info(
                "Ollama is not reachable on the configured URL; the summary "
                "step will be skipped (transcription file is still produced)."
            )
